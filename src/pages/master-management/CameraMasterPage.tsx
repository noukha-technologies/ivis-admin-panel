...
const CameraMasterPage: React.FC = () => {
  const [cameras, setCameras] = useState<CameraMaster[]>([
    { id: 'CAM-601', name: 'ANPR-MCT-IN-1', type: 'ANPR', code: 'CAM-IN-01', line: 'Line 1 (Light)', description: 'Main entrance ANPR', status: 'Active', created: '2026-01-05', ipAddress: '192.168.1.50', streamUrl: 'rtsp://192.168.1.50/anpr' },
    { id: 'CAM-602', name: 'ANPR-MCT-OUT-1', type: 'ANPR', code: 'CAM-OUT-01', line: 'Line 1 (Light)', description: 'Main exit ANPR', status: 'Active', created: '2026-01-05', ipAddress: '192.168.1.51', streamUrl: 'rtsp://192.168.1.51/anpr' },
    { id: 'CAM-603', name: 'ANPR-SLL-IN-1', type: 'CCTV', code: 'CAM-SLL-01', line: 'Line 3 (Mixed)', description: 'Salalah entrance', status: 'Active', created: '2026-01-15', ipAddress: '192.168.1.52', streamUrl: 'rtsp://192.168.1.52/cctv' },
  ]);

  const linesList = ['Line 1 (Light)', 'Line 2 (Heavy)', 'Line 3 (Mixed)', 'Line 4 (Bikes)'];
  ...
  const [selectedItem, setSelectedItem] = useState<CameraMaster | null>(null);
  ...
  const handleOpenEdit = (item: CameraMaster) => {
  ...
  const handleOpenView = (item: CameraMaster) => {
    setSelectedItem(item);
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    const generatedId = `CAM-${Math.floor(604 + Math.random() * 900)}`;
    const formattedDate = new Date().toISOString().split('T')[0];

    const newCamera: CameraMaster = {
      id: generatedId,
      name: formData.name || 'Unnamed',
      type: formData.type || 'ANPR',
      code: formData.code || '',
              ...
              <tbody>
                {paginatedList.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 transition-colors duration-150 hover:bg-gray-50/80 bg-white group">
                    ...
                    <td className="px-6 py-4.5 text-sm text-gray-600 font-medium truncate max-w-50" title={item.description || item.type}>
        ...
        <div className="fixed inset-0 bg-[#0b0f19]/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-125 border border-neutral-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4.5 border-b border-neutral-100 flex items-center justify-between">
              <h3 className="text-[17px] font-bold text-neutral-800">Camera & ANPR Details</h3>
              <button
            ...
            <div className="p-6 space-y-4 text-[14px]">
              ...
              <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-neutral-50">
                ...
                <span className="col-span-2 text-neutral-800">{selectedItem.type || '—'}</span>
              </div>
              <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-neutral-50">
              ...
              {selectedItem.centerCode && (
                <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-neutral-50">
                  <span className="text-gray-400 font-medium">Center Code</span>
                  <span className="col-span-2 text-neutral-800 font-mono">{selectedItem.centerCode}</span>
                </div>
              )}
              ...
              {selectedItem.streamUrl && (
                <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-neutral-50">
                  ...
                  <span className="col-span-2 text-neutral-600 font-mono text-[12.5px] break-all">{selectedItem.streamUrl}</span>
                </div>
              )}
              <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-neutral-50">
            ...
            <form onSubmit={handleCreate}>
              <div className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-[13px] font-semibold text-[#344054] mb-1.5">Stream URL</label>
                    <input
                      type="text"
                      placeholder="e.g. rtsp://..."
        ...
        <div className="fixed inset-0 bg-[#0b0f19]/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-100 border border-neutral-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <h3 className="text-[17px] font-bold text-neutral-800 mb-1">Delete Camera Setup</h3>
