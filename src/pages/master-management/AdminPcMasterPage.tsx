...
const AdminPcMasterPage: React.FC = () => {
  ...
  const centresList = ['Muscat Main Hub', 'Salalah Centre', 'Sohar Branch', 'Nizwa Station'];
  ...
  const [selectedItem, setSelectedItem] = useState<AdminPcMaster | null>(null);
  ...
  const handleOpenEdit = (item: AdminPcMaster) => {
  ...
  const handleOpenView = (item: AdminPcMaster) => {
    setSelectedItem(item);
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    const generatedId = `P-${Math.floor(505 + Math.random() * 900)}`;
    const formattedDate = new Date().toISOString().split('T')[0];

    const newPc: AdminPcMaster = {
      id: generatedId,
  ...
  const filteredList = pcs.filter((item) => {
    ...
    return (
      ...
      (item.assignedUser && item.assignedUser.toLowerCase().includes(q))
    );
  });
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100 bg-[#F9FAFB]">
                  ...
                  <th className="px-5 py-3 text-[14px] text-[#667085] font-semibold" style={{ padding: '12px 20px' }}>IP Address</th>
